...
adas_1_score <- readr::read_delim("~/ADASSCORES_22May2024.csv") # edit to file name and directory for ADAS Scores, ADNI 1
adas_2_3_go <- readr::read_delim("~/ADAS_ADNIGO23_22May2024.csv") %>% # edit to file name and directory for ADAS Scores, ADNI GO/2/3
  dplyr::mutate(ADAS13=TOTAL13,EXAMDATE=VISDATE,VISCODE=VISCODE2)
...
adas_scores <- dplyr::bind_rows(adas_2_3_go,adas_1_score) %>%
  dplyr::select(RID,VISCODE,EXAMDATE,ADAS13) %>%
